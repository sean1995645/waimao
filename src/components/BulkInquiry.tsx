...
const normalizeItems = (value: unknown): InquiryItem[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  return value.reduce<InquiryItem[]>((accumulator, item) => {
...
const readStoredItems = (): InquiryItem[] => {
  try {
...
const writeStoredItems = (items: InquiryItem[]) => {
...
const BulkInquiry: React.FC = () => {
  ...
  const saveItems = (newItems: InquiryItem[]) => {
    const deduped = Array.from(new Map(newItems.map((item) => [item.id, item])).values());
    writeStoredItems(deduped);
    setItems(deduped);
  };
  ...
  const handleWhatsApp = () => {
    const productNames = items.map((item) => getDisplayName(item)).join('\n');
    const message = `${intl.formatMessage({ id: 'inquiry.whatsappIntro' })}\n\n${productNames}\n\n${intl.formatMessage({ id: 'inquiry.whatsappOutro' })}`;
    const phoneNumber = '8613800102400';
          ...
          <div className="flex items-center gap-3 max-md:gap-2 flex-1 min-w-0 max-md:w-full">
            ...
            <div className="flex items-center gap-2 max-md:gap-1.5 overflow-x-auto max-md:flex-1">
              {items.slice(0, 5).map((item, index) => {
                ...
                return (
                  <div key={item.id} className="relative group flex-shrink-0 motion-safe:animate-in motion-safe:fade-in motion-safe:zoom-in-95 motion-safe:duration-500" style={{ animationDelay: `${index * 70}ms` }}>
                    {item.img ? (
                      <img src={item.img} alt={displayName} className="w-20 h-20 max-md:w-14 max-md:h-14 rounded-lg object-cover border-2 border-white/20" />
                    ) : (
                      <span className="inline-flex items-center justify-center w-20 h-20 max-md:w-14 max-md:h-14 rounded-lg bg-white/20 font-semibold text-sm max-md:text-xs">{displayName.charAt(0)}</span>
                    )}
                    <button
                      ...
                      aria-label={intl.formatMessage({ id: 'inquiry.removeItem' })}
                    >
            ...
            <button
              ...
              onClick={openModal}
            >
              {intl.formatMessage({ id: 'inquiry.viewAll' })}
            </button>
          </div>
          <div className="flex items-center gap-3 max-md:gap-2 flex-shrink-0 max-md:w-full">
            <button className="px-4 max-md:px-3 py-2 max-md:py-1.5 bg-transparent border-[1.5px] border-white/30 text-white/75 rounded-md cursor-pointer text-sm max-md:text-xs transition-all duration-200 max-md:flex-1 hover:border-white/70 hover:text-white" onClick={clearAll}>{intl.formatMessage({ id: 'inquiry.clear' })}</button>
            <button className="inline-flex items-center justify-center gap-2 px-5 max-md:px-3 py-2.5 max-md:py-2 bg-[#25D366] text-white border-none rounded-md font-bold text-sm max-md:text-xs cursor-pointer transition-all duration-200 max-md:flex-1 hover:bg-[#1fb855] hover:-translate-y-px" onClick={handleWhatsApp}>
              ...
              <span className="max-md:hidden">{intl.formatMessage({ id: 'inquiry.whatsapp' })}</span>
            </button>
            <button className="inline-flex items-center justify-center gap-2 px-5 max-md:px-3 py-2.5 max-md:py-2 bg-white text-hn-primary border-none rounded-md font-bold text-sm max-md:text-xs cursor-pointer transition-all duration-200 max-md:flex-1 hover:bg-gray-100 hover:-translate-y-px" onClick={handleInquiry}>
              ...
              <span className="max-md:hidden">{intl.formatMessage({ id: 'inquiry.inquiryNow' })}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[1000] flex items-end justify-center bg-black/50 opacity-100 transition-opacity duration-300 motion-safe:animate-in motion-safe:fade-in motion-safe:duration-200" onClick={closeModal}>
          <div className="relative bg-white rounded-t-3xl max-md:rounded-t-2xl w-full max-w-[960px] max-md:max-w-full max-h-[86vh] max-md:max-h-[68vh] overflow-y-auto px-10 max-md:px-4 pt-8 max-md:pt-4 pb-10 max-md:pb-6 shadow-[0_-12px_40px_rgba(0,0,0,0.18)] animate-modalSlideIn" onClick={(e) => e.stopPropagation()}>
            ...
            <button className="absolute top-5 max-md:top-3 right-5 max-md:right-3 w-9 max-md:w-7 h-9 max-md:h-7 rounded-lg border-none bg-gray-100 text-gray-500 cursor-pointer flex items-center justify-center transition-all duration-200 hover:bg-hn-primary hover:text-white" onClick={closeModal} aria-label={intl.formatMessage({ id: 'inquiry.close' })}>
            ...
            <div className="flex max-md:flex-col flex-row gap-6 max-md:gap-3 items-start justify-center">
              <div className="flex-1 flex flex-wrap gap-5 max-md:gap-2.5 p-6 max-md:p-3 bg-[#f6f8fb] rounded-xl max-md:rounded-lg items-center justify-center">
                {items.map((item, index) => {
                  ...
                  return (
                    <div key={item.id} className="flex flex-col items-center gap-3 max-md:gap-2 p-6 max-md:p-2.5 bg-white border border-[#dde4ec] rounded-xl max-md:rounded-lg text-sm transition-all duration-200 relative w-[200px] max-md:w-[110px] hover:border-hn-accent hover:shadow-[0_8px_20px_rgba(13,111,184,0.16)]">
                      {item.img ? (
                        <img src={item.img} alt={displayName} className="w-[180px] max-md:w-[90px] h-[180px] max-md:h-[90px] object-cover rounded-lg" />
                      ) : (
                        <div className="w-[180px] max-md:w-[90px] h-[180px] max-md:h-[90px] bg-gray-200 rounded-lg flex items-center justify-center font-semibold text-gray-500 text-[3rem] max-md:text-xl">{displayName.charAt(0)}</div>
                      )}
                      <p className="w-full text-center text-[0.84rem] font-semibold text-hn-primary">{displayName}</p>
                      <button
              ...
              <div className="flex-shrink-0 w-[200px] max-md:w-full flex flex-col gap-3 max-md:gap-2">
                <button className="inline-flex items-center justify-center gap-2 w-full px-4 py-3.5 max-md:py-2.5 text-white border-none rounded-lg font-bold cursor-pointer transition-all duration-200 bg-[#25D366] hover:bg-[#1fb855] hover:-translate-y-0.5" onClick={handleWhatsApp}>
                  ...
                  {intl.formatMessage({ id: 'inquiry.whatsapp' })}
                </button>
                <button className="inline-flex items-center justify-center gap-2 w-full px-4 py-3.5 max-md:py-2.5 text-white border-none rounded-lg font-bold cursor-pointer transition-all duration-200 bg-[#0d6fb8] hover:bg-[#0b5fa0] hover:-translate-y-0.5" onClick={handleInquiry}>
                  ...
                  {intl.formatMessage({ id: 'inquiry.inquiryNow' })}
                </button>
              </div>
            </div>
          </div>
...
export const addToInquiry = (item: InquiryItem) => {
  try {
    ...
    if (!items.find((existingItem) => existingItem.id === item.id)) {
      writeStoredItems([...items, item]);
    }
  } catch (error) {
    console.error('Failed to add item to inquiry:', error);
  }
};

export const removeFromInquiry = (id: string) => {
  try {
    const items = readStoredItems();
    writeStoredItems(items.filter((item) => item.id !== id));
  } catch (error) {
    console.error('Failed to remove item from inquiry:', error);
  }
};

export const isInInquiry = (id: string): boolean => {
  try {
    return readStoredItems().some((item) => item.id === id);
  } catch (error) {
    return false;
  }
};
