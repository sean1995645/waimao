...
const PageHero: React.FC<PageHeroProps> = ({
  ...
  return (
    <section className="relative isolate overflow-hidden bg-[#0c192d] text-white">
      ...
      <div className="absolute -right-28 -top-24 h-72 w-72 rounded-full blur-[48px] opacity-35 motion-safe:animate-floatSoft" style={{ background: `radial-gradient(circle, ${accentColor} 0%, transparent 72%)` }}></div>
      <div className="absolute -left-24 -bottom-36 h-64 w-64 rounded-full bg-white/10 blur-[48px] opacity-25 motion-safe:animate-driftX"></div>
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto grid gap-5 sm:gap-0 px-4 sm:px-6 py-8 sm:py-20 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-end lg:gap-10 lg:px-5 lg:py-24">
        <div className="max-w-[42rem] motion-fade-right">
          <span className="inline-flex items-center rounded-full border border-white/15 bg-white/10 px-2.5 py-0.5 text-[0.7rem] sm:text-[0.78rem] font-semibold uppercase tracking-[0.15em] text-[#c0d6e8] backdrop-blur-sm mb-2.5 sm:mb-4 sm:px-3 sm:py-1 motion-safe:animate-pulseGlow">{eyebrow}</span>
        ...
        {(panelTitle || panelText || stats) && (
          <div className="motion-fade-left rounded-xl sm:rounded-3xl border border-white/12 bg-white/10 px-3.5 sm:px-6 py-3.5 sm:py-6 shadow-[0_20px_70px_rgba(0,0,0,0.35)] backdrop-blur-xl">
            ...
            {stats && stats.length > 0 && (
              <div className="mt-3 sm:mt-5 lg:mt-5 grid grid-cols-3 lg:grid-cols-1 gap-2 sm:gap-2 lg:gap-3">
                {stats.map((stat, index) => (
                  <div key={index} className="rounded-lg sm:rounded-[1.1rem] border border-white/10 bg-[rgba(13,29,52,0.55)] px-2.5 sm:px-4 py-2.5 sm:py-4 motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-3 motion-safe:duration-700" style={{ animationDelay: `${index * 90}ms` }}>
